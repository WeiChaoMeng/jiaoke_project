...
import com.jiaoke.qualitytest.dao.report.VQualityTestReportXjlDao;
...
/**
 * VQualityTestReportXjl的服务接口的实现类
 * 
 * @author 
 *
 */
@Service
public class VQualityTestReportXjlServiceImpl implements VQualityTestReportXjlService {
	private final Logger LOG = LogManager.getLogger(this.getClass());

	@Autowired
	private VQualityTestReportXjlDao vQualityTestReportXjlDao;
	...
	public void setSeachFilter(Assist assist, VQualityTestReportXjl value) {
	...
	@Override
	public String find(VQualityTestReportXjl value) {
		...
		List<VQualityTestReportXjl> result = vQualityTestReportXjlDao.selectVQualityTestReportXjl(assist);
		if (LOG.isDebugEnabled()) {
			LOG.debug("执行获取VQualityTestReportKf数据集-->结果:", result);
		}
		Assist assistCount = new Assist();
		setSeachFilter(assistCount, value);
		int count = (int) vQualityTestReportXjlDao.getVQualityTestReportXjlRowCount(assistCount);

		LayUIPage ResultPage = new LayUIPage<VQualityTestReportXjl>(0, "OK", count, result);
		return ResultPage.toJson();
	}


	@Override
	public String saveNotNull(VQualityTestReportXjl value) {
		if (value == null) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("执行将VQualityTestReportXjl中属性值不为null的数据保存到数据库-->失败:对象不能为空");
			}
			return resultFormat(C412, null);
		}
		int result = vQualityTestReportXjlDao.insertNotNullVQualityTestReportXjl(value);
		if (LOG.isDebugEnabled()) {
			LOG.debug("执行将VQualityTestReportXjl中属性值不为null的数据保存到数据库-->结果:", result);
		}
