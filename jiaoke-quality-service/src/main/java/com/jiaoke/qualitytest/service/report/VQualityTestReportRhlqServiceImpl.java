...
import com.jiaoke.qualitytest.dao.report.VQualityTestReportRhlqDao;
...
/**
 * VQualityTestReportRhlq的服务接口的实现类
 * 
 * @author 
 *
 */
@Service
public class VQualityTestReportRhlqServiceImpl implements VQualityTestReportRhlqService {
	private final Logger LOG = LogManager.getLogger(this.getClass());

	@Autowired
	private VQualityTestReportRhlqDao vQualityTestReportRhlqDao;
	...
	public void setSeachFilter(Assist assist, VQualityTestReportRhlq value) {
	...
	@Override
	public String find(VQualityTestReportRhlq value) {
		...
		List<VQualityTestReportRhlq> result = vQualityTestReportRhlqDao.selectVQualityTestReportRhlq(assist);
		if (LOG.isDebugEnabled()) {
			LOG.debug("执行获取VQualityTestReportKf数据集-->结果:", result);
		}
		Assist assistCount = new Assist();
		setSeachFilter(assistCount, value);
		int count = (int) vQualityTestReportRhlqDao.getVQualityTestReportRhlqRowCount(assistCount);

		LayUIPage ResultPage = new LayUIPage<VQualityTestReportRhlq>(0, "OK", count, result);
		return ResultPage.toJson();
	}

	@Override
	public String saveNotNull(VQualityTestReportRhlq value) {
		if (value == null) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("执行将VQualityTestReportRhlq中属性值不为null的数据保存到数据库-->失败:对象不能为空");
			}
			return resultFormat(C412, null);
		}
		int result = vQualityTestReportRhlqDao.insertNotNullVQualityTestReportRhlq(value);
		if (LOG.isDebugEnabled()) {
			LOG.debug("执行将VQualityTestReportRhlq中属性值不为null的数据保存到数据库-->结果:", result);
		}
