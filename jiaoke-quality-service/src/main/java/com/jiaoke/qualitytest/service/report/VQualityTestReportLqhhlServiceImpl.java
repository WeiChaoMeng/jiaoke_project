...
import com.jiaoke.qualitytest.dao.report.VQualityTestReportLqhhlDao;
...
/**
 * VQualityTestReportLqhhl的服务接口的实现类
 * 
 * @author 
 *
 */
@Service
public class VQualityTestReportLqhhlServiceImpl implements VQualityTestReportLqhhlService {
	private final Logger LOG = LogManager.getLogger(this.getClass());

	@Autowired
	private VQualityTestReportLqhhlDao vQualityTestReportLqhhlDao;
	...
	public void setSeachFilter(Assist assist, VQualityTestReportLqhhl value) {
	...
	@Override
	public String find(VQualityTestReportLqhhl value) {
		...
		List<VQualityTestReportLqhhl> result = vQualityTestReportLqhhlDao.selectVQualityTestReportLqhhl(assist);
		if (LOG.isDebugEnabled()) {
			LOG.debug("执行获取VQualityTestReportKf数据集-->结果:", result);
		}
		Assist assistCount = new Assist();
		setSeachFilter(assistCount, value);
		int count = (int) vQualityTestReportLqhhlDao.getVQualityTestReportLqhhlRowCount(assistCount);

		LayUIPage ResultPage = new LayUIPage<VQualityTestReportLqhhl>(0, "OK", count, result);
	...
	@Override
	public String saveNotNull(VQualityTestReportLqhhl value) {
		if (value == null) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("执行将VQualityTestReportLqhhl中属性值不为null的数据保存到数据库-->失败:对象不能为空");
			}
			return resultFormat(C412, null);
		}
		int result = vQualityTestReportLqhhlDao.insertNotNullVQualityTestReportLqhhl(value);
		if (LOG.isDebugEnabled()) {
			LOG.debug("执行将VQualityTestReportLqhhl中属性值不为null的数据保存到数据库-->结果:", result);
		}
