...
public class OaActContractSignController {
    ...
    public String toEdit(String id, Model model) {
        OaActContractSign oaActContractSign = oaActContractSignService.selectByPrimaryKey(id);
        model.addAttribute("oaActContractSign", oaActContractSign);
        model.addAttribute("annexList", JsonHelper.toJSONString(oaActContractSign.getAnnex()));
    ...
    public String editAdd(OaActContractSign oaActContractSign) {
        ...
        if (oaActContractSignService.edit(oaActContractSign) < 0) {
            ...
            String instance = activitiUtil.startProcessInstanceByKey("oa_meals", "oa_act_meals:" + oaActContractSign.getId(), map, getCurrentUser().getId().toString());
            if (instance != null) {
                //发送成功后更新状态
    ...
    public String deleteAnnexes(String[] array, String id) {
        if (oaActContractSignService.updateAnnexes(array, id) < 1) {
            return "error";
        }
        return "success";
    }
