...
public class OaActRelieveLaborContractController {
    ...
    public String toEdit(String id, Model model) {
        OaActRelieveLaborContract oaActRelieveLaborContract = oaActRelieveLaborContractService.selectByPrimaryKey(id);
        model.addAttribute("oaActRelieveLaborContract", oaActRelieveLaborContract);
        model.addAttribute("annexList", JsonHelper.toJSONString(oaActRelieveLaborContract.getAnnex()));
    ...
    public String editAdd(OaActRelieveLaborContract oaActRelieveLaborContract) {
        ...
        if (oaActRelieveLaborContractService.edit(oaActRelieveLaborContract) < 0) {
            ...
            String instance = activitiUtil.startProcessInstanceByKey("oa_meals", "oa_act_meals:" + oaActRelieveLaborContract.getId(), map, getCurrentUser().getId().toString());
            if (instance != null) {
                //发送成功后更新状态
    ...
    public String deleteAnnexes(String[] array, String id) {
        if (oaActRelieveLaborContractService.updateAnnexes(array, id) < 1) {
            return "error";
        }
        return "success";
    }
