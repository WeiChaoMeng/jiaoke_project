...
public class OaActProductionLeaveController {
    ...
    public String toEdit(String id, Model model) {
        OaActProductionLeave oaActProductionLeave = oaActProductionLeaveService.selectByPrimaryKey(id);
        model.addAttribute("oaActProductionLeave", oaActProductionLeave);
        model.addAttribute("annexList", JsonHelper.toJSONString(oaActProductionLeave.getAnnex()));
    ...
    public String editAdd(OaActProductionLeave oaActProductionLeave) {
        ...
        if (oaActProductionLeaveService.edit(oaActProductionLeave) < 0) {
            ...
            String instance = activitiUtil.startProcessInstanceByKey("oa_meals", "oa_act_meals:" + oaActProductionLeave.getId(), map, getCurrentUser().getId().toString());
            if (instance != null) {
                //发送成功后更新状态
    ...
    public String deleteAnnexes(String[] array, String id) {
        if (oaActProductionLeaveService.updateAnnexes(array, id) < 1) {
            return "error";
        }
        return "success";
    }
