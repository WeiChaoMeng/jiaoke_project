...
public class OaActPactSignController {
    ...
    public String toEdit(String id, Model model) {
        OaActPactSign oaActPactSign = oaActPactSignService.selectByPrimaryKey(id);
        model.addAttribute("oaActPactSign", oaActPactSign);
        model.addAttribute("annexList", JsonHelper.toJSONString(oaActPactSign.getAnnex()));
    ...
    public String editAdd(OaActPactSign oaActPactSign) {
        ...
        if (oaActPactSignService.edit(oaActPactSign) < 0) {
            ...
            String instance = activitiUtil.startProcessInstanceByKey("oa_meals", "oa_act_meals:" + oaActPactSign.getId(), map, getCurrentUser().getId().toString());
            if (instance != null) {
                //发送成功后更新状态
    ...
    public String deleteAnnexes(String[] array, String id) {
        if (oaActPactSignService.updateAnnexes(array, id) < 1) {
            return "error";
        }
        return "success";
    }
