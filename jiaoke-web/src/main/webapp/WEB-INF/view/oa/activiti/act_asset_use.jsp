...
<form id="oaDocumentProcessing" action="http://127.0.0.1:8080/document/add" method="post">
    ...
    <table class="formtable">
        ...
        <tr>
            <td class="tlabel">资产名称：</td>
            <td id="assetName">

            </td>
            <td class="tlabel">剩余数量：</td>
            <td id="surplusQuantity">

            </td>
            <td class="tlabel">领用数量：</td>
            <td>
                <div class="data">
                    <input type="text" class="forminput inputstyle" id="useNumber" name="useNumber" value="" oninput="value=value.replace(/[^\d]/g,'')">
                </div>
            </td>
        </tr>
...
<script>
    ...
    $(function () {
        ...
        var assetNameInfo = '<select class="select swidth" id="assetNameSelect" name="state" data-value="0" required>\n' +
            '<option value="0">请选择</option>\n' +
            '<option value="1">得力</option>\n' +
            '<option value="2">雀巢</option>\n' +
            '<option value="3">晨光</option>\n' +
            '</select>';
        $('#assetName').html(assetNameInfo);

        var surplusQuantityInfo = '<select class="select swidth" id="surplusQuantitySelect" name="state" data-value="0" disabled style="-webkit-appearance: none;text-indent: 10px;">\n' +
            '<option value="0">请选择</option>\n' +
            '<option value="1">12</option>\n' +
            '<option value="2">23</option>\n' +
            '<option value="3">9</option>\n' +
            '</select>';
        $('#surplusQuantity').html(surplusQuantityInfo);
    });

    $(document).ready(function () {
        $('#assetNameSelect').change(function () {
            let assetNameOption = $('#assetNameSelect').val()
            $('#surplusQuantitySelect').val(assetNameOption);
        });
    });
