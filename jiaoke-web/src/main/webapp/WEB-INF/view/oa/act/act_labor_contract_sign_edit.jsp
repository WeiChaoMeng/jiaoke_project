                ...
                <td style="width: 32%">
                    <div class="common_input_frame">
                        <input type="text" id="title" name="title" placeholder="请输入标题" title="点击此处填写标题"
                               value="${oaActLaborContractSign.title}" autocomplete="off">
                    </div>
                </td>
    ...
    <div>
        <div class="notice-content">
            <input type="text" class="notice-personnel-field" onclick="recipientSelect()"
                   value="${oaActLaborContractSign.notifiedPersonStr}" id="recipientName" readonly>将与我公司签订劳动合同，请各部门（领导）签署意见。
...
<script>
    ...
    function savePending() {
        ...
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/laborContractSign/edit',
                data: $('#oaActLaborContractSign').serialize(),
                error: function (request) {
                    layer.msg("出错！");
                },
