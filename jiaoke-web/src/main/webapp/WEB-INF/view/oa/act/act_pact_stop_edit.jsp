...
<form id="oaActPactStop">
            ...
            <tr>
                ...
                <td style="width: 44%">
                    <div class="common_input_frame">
                        <input type="text" id="title" name="title" placeholder="请输入标题" title="点击此处填写标题" value="${oaActPactStop.title}" autocomplete="off">
                    </div>
                </td>
    ...
    <div>
        <div class="notice-personnel">
            <input type="text" class="notice-personnel-field" name="name" value="${oaActPactStop.name}" autocomplete="off">:
        </div>

        <div class="notice-content">
            你与我公司签订的协议期为
            <input class="agreement-start-end start-date" type="text" name="startDate" value="${oaActPactStop.startDate}" onfocus="this.blur()">至
            <input class="agreement-start-end end-date" type="text" name="endDate" value="${oaActPactStop.endDate}" onfocus="this.blur()">的劳务协议将于
            <input type="text" class="agreement-days" name="number" value="${oaActPactStop.number}" oninput="value=value.replace(/^(0+)|[^\d]/g,'')"
                   autocomplete="off">天后到期。（附《劳务协议续签个人意向》）
            <input type="hidden" id="annex" name="annex">
            <input type="hidden" id="id" name="id" value="${oaActPactStop.id}">
        </div>
        ...
        <div class="notice-sign">
            <div class="notice-sign-position">
                ...
                <input class="notice-date" type="text" name="noticeDate" value="${oaActPactStop.noticeDate}" readonly>
            </div>
        </div>
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result === "success") {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '${path}/pactStop/deleteAnnexes',
