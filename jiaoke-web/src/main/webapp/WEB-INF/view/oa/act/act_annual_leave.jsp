...
<form id="oaActAnnualLeave">
                ...
                <td style="width: 44%">
                    <div class="common_input_frame">
                        <input type="text" id="title" name="title" placeholder="请输入标题" title="点击此处填写标题"
                               value="年休假审批表(${nickname} <%=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())%>)"
                               autocomplete="off">
                    </div>
                </td>
    ...
    <span class="fill-in-date">填表日期：<%=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())%></span>
    ...
    <table class="formTable">
        ...
        <tr>
            ...
            <td class="table-td-content">
                <input type="text" class="formInput" name="total" oninput="value=value.replace(/^(0+)|[^\d]/g,'')"
                       autocomplete="off">
            </td>

            <td class="tdLabel">计划休假时间</td>
            <td class="table-td-content">
                <input type="text" class="formInput apply-date" name="planDateStr" onfocus="this.blur()">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="table-td-content">
                <input type="text" class="formInput-readonly" name="entertain" value="${nickname}" readonly>
            </td>

            <td class="tdLabel">主管</td>
            <td class="table-td-content">
                <input type="text" class="formInput-readonly" name="applicant" readonly>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="table-td-content">
                <input type="text" class="formInput-readonly" name="entertain" readonly>
            </td>
...
<script>
    ...
    function savePending() {
        ...
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/annualLeave/savePending',
                data: $('#oaActAnnualLeave').serialize(),
                error: function (request) {
                    layer.msg("出错！");
                },
