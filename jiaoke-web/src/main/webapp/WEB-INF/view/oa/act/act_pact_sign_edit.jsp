...
<head>
    <meta charset="utf-8">
    <title>编辑劳务派遣员工协议签订、续订、变更、终止审批表</title>
...
<div class="table-title">
    <span>编辑外包人员协议签订、续订、变更、终止审批表</span>
</div>
    ...
    <div class="form_area" id="titleArea" style="margin-bottom: 15px">
        <table>
            ...
            <tr>
                ...
                <td style="width: 39%">
                    <div class="common_input_frame">
                        <input type="text" id="title" name="title" placeholder="请输入标题" title="点击此处填写标题"
                               value="${oaActPactSign.title}" autocomplete="off">
                    </div>
                </td>
                ...
                <td>
                    <div class="common_input_frame">
                        <input type="text" placeholder="部门负责人→部门主管领导→人事部门→总经理→通知人、被通知人(知会)" readonly="readonly">
                    </div>
                </td>
            </tr>
            </tbody>
    ...
    <div>
        <div class="notice-content">
            <input type="text" class="notice-personnel-field" onclick="recipientSelect()"
                   value="${oaActPactSign.notifiedPersonStr}" id="recipientName" readonly>与外包公司签订的劳务协议将于
            <input class="agreement-start-end start-date" type="text" name="startDate"
                   value="${oaActPactSign.startDate}" onfocus="this.blur()">到期，请各部门（领导）于
            <input class="agreement-start-end end-date" type="text" name="endDate" value="${oaActPactSign.endDate}"
                   onfocus="this.blur()">前签署意见。
            <input type="hidden" id="annex" name="annex">
            <input type="hidden" id="id" name="id" value="${oaActPactSign.id}">
            <input type="hidden" id="recipientId" name="notifiedPerson" value="${oaActPactSign.notifiedPerson}">
    ...
    function send() {
        if ($.trim($("#title").val()) === '') {
            ...
            if ($.trim($("#recipientId").val()) === '') {
                ...
                $.ajax({
                    type: "POST",
                    url: '${path}/pactSign/getNotifiedPersonPrincipal',
