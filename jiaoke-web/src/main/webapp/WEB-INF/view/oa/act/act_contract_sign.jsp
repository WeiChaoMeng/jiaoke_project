    ...
    <div class="form_area" id="titleArea" style="margin-bottom: 15px">
        <table>
            ...
            <tr>
                ...
                <td>
                    <div class="common_input_frame">
                        <input type="text" placeholder="部门负责人→部门主管领导→劳资部门→总经理→通知人、被通知人(知会)"
                               readonly="readonly">
                    </div>
                </td>
            </tr>
...
<script>
    ...
    function send() {
        if ($.trim($("#title").val()) === '') {
            ...
            if ($.trim($("#recipientId").val()) === '') {
                ...
                $.ajax({
                    type: "POST",
                    url: '${path}/contractSign/getNotifiedPersonPrincipal',
    ...
    function savePending() {
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/contractSign/savePending',
                data: $('#oaActContractSign').serialize(),
                error: function (request) {
                    layer.msg("出错！");
                },
