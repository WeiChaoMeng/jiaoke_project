        ...
        <c:when test="${oaActAcceptanceWarehousing.associatedId != ''}">
            <div class="relevance-tool" id="relevanceList">
                <div class="relevance-tool-padding">
                    ...
                    <div id="relevance">
                        <div id="${oaActAcceptanceWarehousing.associatedId}" class="table-file">
                            <div class="relevance-file-content">
                                ...
                                <span class="delete-file" title="删除" onclick="relevanceFormsDelete('${oaActAcceptanceWarehousing.associatedName}','${oaActAcceptanceWarehousing.associatedId}')">&#xeabb;</span>
                                <input type="hidden" value="${oaActAcceptanceWarehousing.associatedId}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <td style="width: 44%">
                    <div class="common_input_frame">
                        <input type="text" id="title" name="title" placeholder="请输入标题" title="点击此处填写标题"
                               value="${oaActAcceptanceWarehousing.title}" autocomplete="off">
                    </div>
                </td>
    ...
    <table class="formTable"  style="margin: 0">
        ...
        <c:forEach items="${oaActAcceptanceWarehousing.oaAcceptanceWarehousingList}" var="list" varStatus="status">
            <tr>
                ...
                <td class="table-td-content">
                    <input type="text" class="formInput" name="remark" id="remark" value="${list.remark}" autocomplete="off">
                </td>
            </tr>
        </c:forEach>
        </tbody>
...
<script>
    ...
    function send() {
        ...
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/acceptanceWarehousing/editAdd',
    ...
    function savePending() {
        ...
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/acceptanceWarehousing/edit',
                contentType: "application/json;charset=utf-8",
                data: JSON.stringify(oaActAcceptanceWarehousing),
                error: function (request) {
                    window.top.tips("出错！", 6, 2, 1000);
