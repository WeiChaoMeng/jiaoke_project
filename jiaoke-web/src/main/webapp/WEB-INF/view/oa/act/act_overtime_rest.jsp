                ...
                <td style="width: 44%">
                    <div class="common_input_frame">
                        <input type="text" id="title" name="title" placeholder="请输入标题" title="点击此处填写标题"
                               value="加班（倒休）审批表(${nickname} <%=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())%>)"
                               autocomplete="off">
                    </div>
                </td>
        ...
        <tr>
            ...
            <td colspan="5" class="table-td-evaluation">
                ...
                <div class="approval-date">
                    ...
                    <input class="approval-date-input" type="text" value="<%=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())%>" readonly>
                </div>
...
<script>
    ...
    function savePending() {
        ...
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/overtimeRest/savePending',
                data: $('#oaActOvertimeRest').serialize(),
                error: function (request) {
                    layer.msg("出错！");
                },
