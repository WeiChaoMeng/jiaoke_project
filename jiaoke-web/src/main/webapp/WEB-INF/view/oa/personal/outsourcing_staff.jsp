            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px">
                    ...
                    <button class="layui-btn" style="position: absolute;left: 10%">
                        <input id="fileSelect" type="file" onchange="importExcel(this)"
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/wageStatistics/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/wageStatistics/settlementMonthFilter',
            data: {'page': page, 'settlementMonth': settlementMonth},
            async: false,
            success: function (data) {
                $('#page').val(1);
                var userInfos = JSON.parse(data);
    ...
    function parseResult(userInfos) {
        ...
        if (userInfoList.length === 0) {
            ...
            for (let i = 0; i < userInfoList.length; i++) {
                ...
                userInfo += '<td>' + userInfoList[i].totalAmount + '</td>';
                userInfo += '<td>' + userInfoList[i].total + '</td>';
    ...
    function particulars(id) {
        window.location.href = "${path}/wageStatistics/toOutsourcedDetails?wageStatisticsId=" + id;
    }

    //删除
    function del(id) {
        window.top.payrollDelete('/deleteOutsourcingStaff', id, $('#page').val(), itselfFrameId);
    }

    //重载页面
    function reloadPayrollData(page) {
        window.location.href = "${path}/wageStatistics/toOutsourcingStaff?page=" + page;
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/wageStatistics/batchDeleteOutsourcingStaff", ids, itselfFrameId);
    }
