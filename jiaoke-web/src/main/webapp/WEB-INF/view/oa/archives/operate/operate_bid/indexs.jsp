            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px;">
                    ...
                    <div class="layui-input-inline layui-show-xs-block" style="float: right;">
                        <input type="text" id="projectName" placeholder="请输入工程名称" autocomplete="off"
                               class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form" lay-filter="demo">
                        ...
                        <tr>
                            ...
                            <th style="width: 16%">招标单位</th>
                            <th style="width: 17%">工程名称</th>
                            <th style="width: 10%">竞标单位</th>
                            <th style="width: 10%">投标价格</th>
...
<script>
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            result += '<tr>';
            result += '<td colspan="9" style="text-align: center">' + '没有查询到匹配记录' + '</td>';
            result += '</tr>';
        } else {
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td  class="text-too-long">' + resultList[i].tenderee + '</td>';
                result += '<td  class="text-too-long">' + resultList[i].projectName + '</td>';
                result += '<td class="text-too-long">' + resultList[i].biddingUnit + '</td>';
                result += '<td>' + resultList[i].tenderPrice + '</td>';
                result += '<td class="text-too-long">' + resultList[i].proprietor + '</td>';
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/operateBid/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/operateBid/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/operateBid/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/operateBid', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/operateBid/batchDelete", ids, itselfFrameId);
    }
