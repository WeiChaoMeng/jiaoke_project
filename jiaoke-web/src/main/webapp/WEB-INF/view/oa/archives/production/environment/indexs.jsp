            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px;">
                    ...
                    <div class="layui-input-inline layui-show-xs-block" style="float: right;">
                        <input type="text" id="projectName" placeholder="请输入项目名称" autocomplete="off"
                               class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form" lay-filter="demo">
                        ...
                        <tr>
                            ...
                            <th style="width: 10%">施工单位</th>
                            <th style="width: 10%">治理措施</th>
                            <th style="width: 10%">改造费用</th>
                            <th style="width: 10%">污染物去向</th>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/productionEnvironment/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/productionEnvironment/searchFilter',
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td  class="text-too-long">' + resultList[i].projectName + '</td>';
                result += '<td  class="text-too-long">' + resultList[i].constructionUnit + '</td>';
                result += '<td  class="text-too-long">' + resultList[i].controlMeasures + '</td>';
                result += '<td>' + resultList[i].retrofitCost + '</td>';
                result += '<td  class="text-too-long">' + resultList[i].contaminants + '</td>';
                result +=
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/productionEnvironment/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/productionEnvironment/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/productionEnvironment/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/productionEnvironment', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/productionEnvironment/batchDelete", ids, itselfFrameId);
    }
