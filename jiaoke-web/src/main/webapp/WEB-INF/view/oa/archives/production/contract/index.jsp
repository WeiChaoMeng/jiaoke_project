            ...
            <td>
                <div>
                    <div class="conditional-query cursor_hand">
                        <input type="text" id="purchaser" class="search-bar" placeholder="合同名称" autocomplete="off">
...
<script>
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].storageLocation + '">' + resultList[i].storageLocation + '</td>';
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].remark + '">' + resultList[i].remark + '</td>';
                result += '</tr>';
            }
        }
        $('#tbody').html(result);
    ...
    //重载页面
    function productionContractPageReload(page) {
    ...
    function del() {
        ...
        if (length !== 1) {
            ...
            var id = $("tbody input:checked").val();
            //主页fun
            window.top.deleteProductionContract(id, $('#page').val());
        }
    }
