                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form" lay-filter="demo">
                        ...
                        <tr>
                            ...
                            <th style="width: 19%">客户名称</th>
                            <th style="width: 20%">工程名称</th>
                            <th style="width: 10%">诉讼金额</th>
                            <th style="width: 10%">诉讼</th>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/operateLawsuit/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/operateLawsuit/searchFilter',
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td class="text-too-long">' + resultList[i].customerName + '</td>';
                result += '<td class="text-too-long">' + resultList[i].projectName + '</td>';
                result += '<td>' + resultList[i].lawsuitAmount + '</td>';
                result += '<td>' + resultList[i].state + '</td>';
                result +=
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/operateLawsuit/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/operateLawsuit/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/operateLawsuit/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/operateLawsuit', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/operateLawsuit/batchDelete", ids, itselfFrameId);
    }
