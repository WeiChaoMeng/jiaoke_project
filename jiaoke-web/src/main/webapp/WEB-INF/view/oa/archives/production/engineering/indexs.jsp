...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/productionEngineering/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/productionEngineering/searchFilter',
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td class="text-too-long">' + resultList[i].contractNumber + '</td>';
                result += '<td>' + resultList[i].contractAmount + '</td>';
                result += '<td>' + resultList[i].settlementAmount + '</td>';
                result += '<td>' + resultList[i].completionDateStr + '</td>';
                result +=
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/productionEngineering/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/productionEngineering/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/productionEngineering/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/productionEngineering', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/productionEngineering/batchDelete", ids, itselfFrameId);
    }
