...
<script>
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td class="text-too-long">' + resultList[i].fileName + '</td>';
                result += '<td>' + resultList[i].issueLeader + '</td>';
                result += '<td class="text-too-long">' + resultList[i].summary + '</td>';
                result += '<td>' + resultList[i].releaseDateStr + '</td>';
                result +=
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/operateOther/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/operateOther/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/operateOther/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/operateOther', id, itselfFrameId);
    }

    //重载页面
    function reloadArchivesData(page) {
        window.location.href = "${path}/operateOther/toIndex?page=" + page;
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/operateOther/batchDelete", ids, itselfFrameId);
    }
