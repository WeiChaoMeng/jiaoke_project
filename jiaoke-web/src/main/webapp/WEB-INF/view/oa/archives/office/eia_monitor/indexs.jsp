                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form" lay-filter="demo">
                        ...
                        <tr>
                            ...
                            <th style="width: 22%;">样品名称</th>
                            <th style="width: 21%;">检测单位</th>
                        ...
                        <tbody id="tbody">
                        <tr>
                            <td colspan="7" style="text-align: center;">没有查询到匹配记录</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    function parseList(oaEiaMonitors) {
        ...
        if (oaEiaMonitorList.length === 0) {
            oaEiaMonitor += '<tr>';
            oaEiaMonitor += '<td colspan="7" style="text-align: center">' + '没有查询到匹配记录' + '</td>';
            oaEiaMonitor += '</tr>';
        } else {
            for (let i = 0; i < oaEiaMonitorList.length; i++) {
                oaEiaMonitor += '<tr>';
                oaEiaMonitor += '<td><input type="checkbox" name="show" lay-skin="primary" lay-filter="choose" value="' + oaEiaMonitorList[i].id + '">' +
                    '<div class="layui-unselect layui-form-checkbox" lay-skin="primary"><i class="layui-icon layui-icon-ok"></i></div></td>';
                oaEiaMonitor += '<td>' + (pageNum === 1 ? pageNum + i : (pageNum - 1) * 12 + i + 1) + '</td>';
                oaEiaMonitor += '<td class="text-too-long">' + oaEiaMonitorList[i].reportNumber + '</td>';
                oaEiaMonitor += '<td class="text-too-long">' + oaEiaMonitorList[i].sampleName + '</td>';
                oaEiaMonitor += '<td class="text-too-long">' + oaEiaMonitorList[i].testingCompany + '</td>';
                oaEiaMonitor += '<td>' + oaEiaMonitorList[i].testingDateStr + '</td>';
                oaEiaMonitor +=
                    '<td class="td-manage" style="white-space: nowrap;">\n' +
                    '<button class="layui-btn layui-btn layui-btn-xs" onclick="edit(' + oaEiaMonitorList[i].id + ')"><i class="layui-icon">&#xe642;</i>编辑</button>\n' +
                    '<button class="layui-btn-danger layui-btn layui-btn-xs" onclick="remove(' + oaEiaMonitorList[i].id + ')"><i class="layui-icon">&#xe640;</i>删除</button>\n' +
                    '<button class="layui-btn layui-btn-normal layui-btn-xs" onclick="particulars(' + oaEiaMonitorList[i].id + ')"><i class="layui-icon">&#xe63c;</i>详情</button>\n' +
                    '</td>';
                oaEiaMonitor += '</tr>';
            }
        }
    ...
    //删除
    function remove(id) {
        window.top.deleteArchivesData('/eiaMonitor', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/eiaMonitor/batchDelete", ids, itselfFrameId);
    }
