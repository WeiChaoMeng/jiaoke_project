            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px;">
                    ...
                    <div class="layui-input-inline layui-show-xs-block" style="float: right;">
                        <input type="text" id="purchaser" placeholder="请输入合同名称" autocomplete="off"
                               class="layui-input">
                    </div>
                </div>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/productionContract/loadingData',
            data: {'page': page},
            async: false,
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td>' + resultList[i].relativePerson + '</td>';
                result += '<td>' + resultList[i].amount + '</td>';
                result += '<td class="text-too-long">' + resultList[i].storageLocation + '</td>';
                result += '<td class="text-too-long">' + resultList[i].remark + '</td>';
                result +=
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/productionContract/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/productionContract/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/productionContract/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/productionContract', id, itselfFrameId);
    }

    //重载页面
    function reloadArchivesData(page) {
        window.location.href = "${path}/productionContract/toIndex?page=" + page;
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/productionContract/batchDelete", ids, itselfFrameId);
    }
