            ...
            <td>
                <div>
                    <div class="conditional-query cursor_hand">
                        <input type="text" id="purchaser" class="search-bar" placeholder="购买方" autocomplete="off">
...
<table class="simple_table cursor_hand">
    ...
    <th style="width: 30%;">购买方</th>
    <th style="width: 10%;">签订日期</th>
    <th style="width: 10%;">业务员</th>
    <th style="width: 14%;">合同金额</th>
    <th style="width: 10%;">结清日期</th>
    </thead>
...
<script>
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td style="text-align:left;text-indent:10px">' + resultList[i].numbering + '</td>';
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].purchaser + '">' + resultList[i].purchaser + '</td>';
                result += '<td>' + resultList[i].signingDateStr + '</td>';
                result += '<td>' + resultList[i].salesman + '</td>';
                result += '<td>' + resultList[i].amount + '</td>';
                result += '<td>' + resultList[i].settlementDateStr + '</td>';
                result += '</tr>';
            }
        }
        $('#tbody').html(result);
    ...
    //新增
    function add() {
        window.location.href = '${path}/operate/toAdd?page=' + $('#page').val();
    }

    //详情
    function particulars(id) {
        window.location.href = "${path}/operate/details?id=" + id + "&page=" + $('#page').val();
    }

    //重载页面
    function operateContractPageReload(page) {
        window.location.href = "${path}/operate/toOperateContract?page=" + page;
    }
    ...
    function editDepartment() {
        ...
        if (length !== 1) {
            ...
            window.location.href = "${path}/operate/toEdit?id=" + id + "&page=" + $('#page').val();
        }
    }
    ...
    function del() {
        ...
        if (length !== 1) {
            ...
            var id = $("tbody input:checked").val();
            //主页fun
            window.top.deleteOperateContract(id, $('#page').val());
        }
    }
