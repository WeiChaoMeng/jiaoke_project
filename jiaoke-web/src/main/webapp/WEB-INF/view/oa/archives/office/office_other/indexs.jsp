...
<script>
    ...
    function parseList(OaOfficeOthers) {
        ...
        if (oaOfficeOtherList.length === 0) {
            oaOfficeOther += '<tr>';
            oaOfficeOther += '<td colspan="7" style="text-align: center">' + '没有查询到匹配记录' + '</td>';
            oaOfficeOther += '</tr>';
        } else {
            for (let i = 0; i < oaOfficeOtherList.length; i++) {
                oaOfficeOther += '<tr>';
                oaOfficeOther += '<td><input type="checkbox" name="show" lay-skin="primary" lay-filter="choose" value="' + oaOfficeOtherList[i].id + '">' +
                    '<div class="layui-unselect layui-form-checkbox" lay-skin="primary"><i class="layui-icon layui-icon-ok"></i></div></td>';
                oaOfficeOther += '<td>' + (pageNum === 1 ? pageNum + i : (pageNum - 1) * 12 + i + 1) + '</td>';
                oaOfficeOther += '<td class="text-too-long">' + oaOfficeOtherList[i].numbering + '</td>';
                oaOfficeOther += '<td class="text-too-long">' + oaOfficeOtherList[i].name + '</td>';
                ...
                oaOfficeOther +=
                    ...
                    '<button class="layui-btn layui-btn layui-btn-xs" onclick="edit(' + oaOfficeOtherList[i].id + ')"><i class="layui-icon">&#xe642;</i>编辑</button>\n' +
                    '<button class="layui-btn-danger layui-btn layui-btn-xs" onclick="remove(' + oaOfficeOtherList[i].id + ')"><i class="layui-icon">&#xe640;</i>删除</button>\n' +
                    '<button class="layui-btn layui-btn-normal layui-btn-xs" onclick="particulars(' + oaOfficeOtherList[i].id + ')"><i class="layui-icon">&#xe63c;</i>详情</button>\n' +
                    '</td>';
                oaOfficeOther += '</tr>';
            }
        }
    ...
    //删除
    function remove(id) {
        window.top.deleteArchivesData('/officeOther', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/officeOther/batchDelete", ids, itselfFrameId);
    }
