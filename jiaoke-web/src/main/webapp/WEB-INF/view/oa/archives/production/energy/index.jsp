            ...
            <td>
                <div>
                    <div class="conditional-query cursor_hand">
                        <input type="text" id="yearStr" class="search-bar" placeholder="年份" autocomplete="off">
...
<table class="simple_table cursor_hand">
    ...
    <th style="width: 9%;">年份</th>
    <th style="width: 10%;">产量(吨)</th>
    <th style="width: 10%;">产值(万元)</th>
    <th style="width: 15%;">万元产值能耗(吨标准煤/万元)</th>
    <th style="width: 15%;">单位产品能耗(千克标准煤/吨)</th>
    <th style="width: 10%;">天然气消耗量(万立方米)</th>
    <th style="width: 10%;">耗电量(万千瓦时)</th>
    <th style="width: 15%;">单位产品碳排放量(千克二氧化碳/吨)</th>
    </thead>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/productionEnergy/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/productionEnergy/searchFilter',
            data: {'page': page, 'yearStr': yearStr},
            async: false,
            success: function (data) {
                $('#page').val(1);
                var resultLists = JSON.parse(data);
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            result += '<tr>';
            result += '<td colspan="10">' + '没有查询到匹配记录' + '</td>';
            result += '</tr>';
        } else {
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td>' + resultList[i].yield + '</td>';
                result += '<td>' + resultList[i].outputValue + '</td>';
                result += '<td>' + resultList[i].productionConsume + '</td>';
                result += '<td>' + resultList[i].productConsume + '</td>';
                result += '<td>' + resultList[i].gasConsume + '</td>';
                result += '<td>' + resultList[i].powerConsumption + '</td>';
                result += '<td>' + resultList[i].carbonEmission + '</td>';
                result += '</tr>';
            }
        }
        $('#tbody').html(result);
    ...
    //新增
    function add() {
        window.location.href = '${path}/productionEnergy/toAdd?page=' + $('#page').val();
    }

    //详情
    function particulars(id) {
        window.location.href = "${path}/productionEnergy/details?id=" + id + "&page=" + $('#page').val();
    }
    ...
    function editDepartment() {
        ...
        if (length !== 1) {
            ...
            window.location.href = "${path}/productionEnergy/toEdit?id=" + id + "&page=" + $('#page').val();
        }
    }
