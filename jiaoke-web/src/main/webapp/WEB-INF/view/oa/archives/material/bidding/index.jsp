            ...
            <td>
                <div>
                    <div class="conditional-query cursor_hand">
                        <input type="text" id="file" class="search-bar" placeholder="招标文件" autocomplete="off">
...
<table class="simple_table cursor_hand">
    ...
    <th style="width: 44%;">招标文件</th>
    <th style="width: 25%;">招标评分标准</th>
    <th style="width: 25%;">招标费</th>
    </thead>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/materialBidding/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/materialBidding/searchFilter',
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].file + '">' + resultList[i].file + '</td>';
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].tenderingScore + '">' + resultList[i].tenderingScore + '</td>';
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].biddingFee + '">' + resultList[i].biddingFee + '</td>';
                result += '</tr>';
            }
        }
        $('#tbody').html(result);
    ...
    //新增
    function add() {
        window.location.href = '${path}/materialBidding/toAdd?page=' + $('#page').val();
    }

    //详情
    function particulars(id) {
        window.location.href = "${path}/materialBidding/details?id=" + id + "&page=" + $('#page').val();
    }
    ...
    function editDepartment() {
        ...
        if (length !== 1) {
            ...
            window.location.href = "${path}/materialBidding/toEdit?id=" + id + "&page=" + $('#page').val();
        }
    }
