            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px;">
                    ...
                    <div class="layui-input-inline layui-show-xs-block" style="float: right;">
                        <input type="text" id="name" placeholder="请输入合同名称" autocomplete="off"
                               class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form" lay-filter="demo">
                        ...
                        <tr>
                            ...
                            <th style="width: 11%;">合同名称</th>
                            <th style="width: 11%;">供货商名称</th>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/materialPurchase/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/materialPurchase/searchFilter',
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td class="text-too-long">' + resultList[i].supplier + '</td>';
                result += '<td class="text-too-long">' + resultList[i].material + '</td>';
                result += '<td>' + resultList[i].number + '</td>';
                result += '<td>' + resultList[i].unitPrice + '</td>';
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/materialPurchase/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/materialPurchase/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/materialPurchase/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/materialPurchase', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/materialPurchase/batchDelete", ids, itselfFrameId);
    }
