            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px;">
                    ...
                    <div class="layui-input-inline layui-show-xs-block" style="float: right;">
                        <input type="text" id="remarks" placeholder="请输入号段" autocomplete="off"
                               class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form" lay-filter="demo">
                        ...
                        <tr>
                            ...
                            <th style="width: 33%">号段</th>
                            <th style="width: 20%">张数</th>
                            <th style="width: 20%">开票日期</th>
...
<script>
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/financeBil/searchFilter',
            data: {'page': page, 'remarks': remarks},
            async: false,
            success: function (data) {
                $('#page').val(1);
                var resultLists = JSON.parse(data);
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td class="text-too-long">' + resultList[i].numberSegment + '</td>';
                result += '<td>' + resultList[i].number + '</td>';
                result += '<td>' + resultList[i].invoiceDateStr + '</td>';
                result +=
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/financeBil/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/financeBil/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/financeBil/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/financeBil', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/financeBil/batchDelete", ids, itselfFrameId);
    }
