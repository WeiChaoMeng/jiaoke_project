            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px;">
                    ...
                    <div class="layui-input-inline layui-show-xs-block" style="float: right;">
                        <input type="text" id="deviceName" placeholder="请输入设备名称" autocomplete="off"
                               class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form" lay-filter="demo">
                        ...
                        <tr>
                            ...
                            <th style="width: 10%">厂家</th>
                            <th style="width: 10%">原值</th>
                            <th style="width: 10%">购入日期</th>
                            <th style="width: 18%">操作</th>
                        </thead>

                        <tbody id="tbody">
                        <tr>
                            <td colspan="10" style="text-align: center;">没有查询到匹配记录</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/productionEquipment/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/productionEquipment/searchFilter',
            data: {'page': page, 'deviceName': deviceName},
            async: false,
            success: function (data) {
                $('#page').val(1);
                var resultLists = JSON.parse(data);
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            result += '<tr>';
            result += '<td colspan="10" style="text-align: center">' + '没有查询到匹配记录' + '</td>';
            result += '</tr>';
        } else {
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td>' + resultList[i].yearStr + '</td>';
                result += '<td>' + resultList[i].fixedAssets + '</td>';
                result += '<td class="text-too-long">' + resultList[i].name + '</td>';
                result += '<td class="text-too-long">' + resultList[i].type + '</td>';
                result += '<td class="text-too-long">' + resultList[i].manufacturer + '</td>';
                result += '<td>' + resultList[i].originalValue + '</td>';
                result += '<td>' + resultList[i].buyDateStr + '</td>';
                result +=
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/productionEquipment/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/productionEquipment/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/productionEquipment/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/productionEquipment', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/productionEquipment/batchDelete", ids, itselfFrameId);
    }
