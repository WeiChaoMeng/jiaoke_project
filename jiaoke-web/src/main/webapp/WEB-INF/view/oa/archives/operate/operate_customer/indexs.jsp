            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px;">
                    ...
                    <div class="layui-input-inline layui-show-xs-block" style="float: right;">
                        <input type="text" id="purchaser" placeholder="请输入工程名称" autocomplete="off"
                               class="layui-input">
                    </div>
                </div>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/operateCustomer/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/operateCustomer/searchFilter',
            data: {'page': page, 'projectName': purchaser},
            async: false,
            success: function (data) {
                $('#page').val(1);
                var resultLists = JSON.parse(data);
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/operateCustomer/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/operateCustomer/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/operateCustomer/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/operateCustomer', id, itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/operateCustomer/batchDelete", ids, itselfFrameId);
    }
