            ...
            <div class="layui-card">
                <div class="layui-card-body" style="min-height: 30px;">
                    ...
                    <div class="layui-input-inline layui-show-xs-block" style="float: right;">
                        <input type="text" id="theme" placeholder="请输入会议主题" autocomplete="off"
                               class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-card-body ">
                    <table class="layui-table layui-form" lay-filter="demo">
                        ...
                        <tr>
                            ...
                            <th style="width: 18%">编号</th>
                            <th style="width: 25%">会议主题</th>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/operateMeeting/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/operateMeeting/searchFilter',
            data: {'page': page, 'theme': theme},
            async: false,
            success: function (data) {
                $('#page').val(1);
                var resultLists = JSON.parse(data);
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            result += '<tr>';
            result += '<td colspan="7" style="text-align: center">' + '没有查询到匹配记录' + '</td>';
            result += '</tr>';
        } else {
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td  class="text-too-long">' + resultList[i].theme + '</td>';
                result += '<td class="text-too-long">' + resultList[i].summary + '</td>';
                result += '<td>' + resultList[i].meetingDateStr + '</td>';
                result +=
    ...
    //新建公司发文
    function add() {
        window.location.href = '${path}/operateMeeting/toAdd?page=' + $('#page').val();
    }

    //公司发文详情
    function particulars(id) {
        window.location.href = "${path}/operateMeeting/details?id=" + id + "&page=" + $('#page').val();
    }

    //修改
    function edit(id) {
        window.location.href = "${path}/operateMeeting/toEdit?id=" + id + "&page=" + $('#page').val();
    }

    //删除
    function remove(id) {
        window.top.deleteArchivesData('/operateMeeting', id, itselfFrameId);
    }

    //重载页面
    function reloadArchivesData(page) {
        window.location.href = "${path}/operateMeeting/toIndex?page=" + page;
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/operateMeeting/batchDelete", ids, itselfFrameId);
    }
