...
<table class="formtable" style="width: 80%;margin: auto">
    ...
    <tr>
        ...
        <td style="width: 240px">
            ...
            <c:if test="${oaDocument.identification == 2}">下行文</c:if>
            <c:if test="${oaDocument.identification == 3}">平行文</c:if>
            <c:if test="${oaDocument.identification == 4}">内部行文</c:if>
            <c:if test="${oaDocument.identification == 5}">外部行文</c:if>
            <c:if test="${oaDocument.identification == 6}">其它</c:if>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
            <c:if test="${oaDocument.draftingDepartment == 1}">办公室</c:if>
            <c:if test="${oaDocument.draftingDepartment == 2}">组织人事部</c:if>
            <c:if test="${oaDocument.draftingDepartment == 3}">党群工作部</c:if>
            <c:if test="${oaDocument.draftingDepartment == 4}">生产经营部</c:if>
            <c:if test="${oaDocument.draftingDepartment == 5}">财务管理部</c:if>
            <c:if test="${oaDocument.draftingDepartment == 6}">企业管理部</c:if>
            <c:if test="${oaDocument.draftingDepartment == 7}">内控审计部</c:if>
            <c:if test="${oaDocument.draftingDepartment == 8}">安全管理部</c:if>
            <c:if test="${oaDocument.draftingDepartment == 9}">纪委监督部</c:if>
        </td>
    ...
    <tr>
        ...
        <td>${oaDocument.issue}</td>
        <td class="tlabel">会签：</td>
        <td colspan="">${oaDocument.countersign}</td>
        <td class="tlabel">保存期限：</td>
        <td>
            <c:if test="${oaDocument.storageLife == 1}">10年</c:if>
            <c:if test="${oaDocument.storageLife == 2}">30年</c:if>
            <c:if test="${oaDocument.storageLife == 3}">永久</c:if>
        </td>
    </tr>

    <tr>
        <td class="tlabel">主题词：</td>
        <td colspan="5">${oaDocument.thematicWords}</td>
    </tr>
...
<div style="text-align: center;padding-top: 20px;">
    <input type="button" value="返回" onclick="previousPage()" style="padding: 7px 20px; background: #92ebff;border: 1px #a5a5a5 solid;">
</div>
