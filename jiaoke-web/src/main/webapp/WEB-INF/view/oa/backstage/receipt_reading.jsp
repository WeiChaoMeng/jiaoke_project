        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-body ">
                    ...
                    <button class="layui-btn" onclick="addMainDelivery()">
...
<script>
        ...
        if (userInfoList.length === 0) {
            ...
            for (let i = 0; i < userInfoList.length; i++) {
                ...
                userInfo +=
                    ...
                    '<button class="layui-btn layui-btn layui-btn-xs" onclick="editMainDelivery(\'' + userInfoList[i].id + '\',\'' + userInfoList[i].name + '\',\'' + userInfoList[i].description + '\')"><i class="layui-icon">&#xe642;</i>编辑</button>\n' +
                    '<button class="layui-btn-danger layui-btn layui-btn-xs" onclick="deleteMainDelivery(' + userInfoList[i].id + ')"><i class="layui-icon">&#xe640;</i>删除</button>\n' +
    ...
    //新增
    function addMainDelivery() {
        window.top.addMainDelivery("/receiptReading", $('#page').val(), itselfFrameId);
    }

    //重载页面
    function parameterConfigurationPage(page) {
        window.location.href = "${path}/receiptReading/toIndex?page=" + page;
    }

    //编辑
    function editMainDelivery(id, name, description) {
        window.top.editMainDelivery(id, name, description, "/receiptReading", $('#page').val(), itselfFrameId);
    }
    ...
    function binding(id) {
        $.ajax({
            type: "post",
            url: '/receiptReading/toBinding',
            data: {'id': id},
            dataType: 'json',
            success: function (data) {
                //主页fun
                window.top.bindingMainDelivery(data, id,"/receiptReading", $('#page').val(), itselfFrameId);
            },
            error: function (result) {
                layer.msg("出错！");
            }
    ...
    //删除
    function deleteMainDelivery(id) {
        window.top.deleteMainDelivery(id, "/receiptReading", itselfFrameId);
    }
    ...
    function delAll() {
        ...
        window.top.batchDeleteData("/receiptReading/batchDeleteData", ids, itselfFrameId);
    }
