layui.use(['form', 'layedit', 'laydate'], function() {
	console.log("qualitytestmanufacturersdictionary_edit");
	...
	myform.getExperimentalInfo = function(id) {
		...
		$.ajax({
			type: "GET",
			async: false,
			url: basePath + "/QualityTestManufacturersDictionary/find/" + id + ".do",
			dataType: 'json',
	...
	myform.Save = function(saveData) {
		...
		$.ajax({
			type: "POST",
			async: false,
			url: basePath + "/QualityTestManufacturersDictionary/update.do",
			data: saveData,
			dataType: 'JSON',
			success: function(msg) {
				if (msg.code = 200) {
