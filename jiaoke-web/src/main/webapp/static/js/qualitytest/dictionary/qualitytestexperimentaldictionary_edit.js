layui.use(['form', 'layedit', 'laydate'], function() {
	console.log("qualitytestexperimentaldictionary_edit");
	...
	myform.getExperimentalInfo = function(id) {
		...
		$.ajax({
			type: "GET",
			async: false,
			url: basePath + "/QualityTestExperimentDictionary/find/" + id + ".do",
			dataType: 'json',
	...
	myform.Save = function(saveData) {
		...
		$.ajax({
			type: "POST",
			async: false,
			url: basePath + "/QualityTestExperimentDictionary/update.do",
			data: saveData,
			dataType: 'JSON',
			success: function(msg) {
				if (msg.code = 200) {
