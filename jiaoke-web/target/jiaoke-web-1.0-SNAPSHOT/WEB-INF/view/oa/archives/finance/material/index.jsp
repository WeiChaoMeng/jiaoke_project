            ...
            <td>
                <div>
                    <div class="conditional-query cursor_hand">
                        <input type="text" id="remarks" class="search-bar" placeholder="备注" autocomplete="off">
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/financeMaterial/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/financeMaterial/searchFilter',
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].remarks + '">' + resultList[i].remarks + '</td>';
    ...
    //新增
    function add() {
        window.location.href = '${path}/financeMaterial/toAdd?page=' + $('#page').val();
    }

    //详情
    function particulars(id) {
        window.location.href = "${path}/financeMaterial/details?id=" + id + "&page=" + $('#page').val();
    }

    //重载页面
    function financeMaterialPageReload(page) {
        window.location.href = "${path}/financeMaterial/toIndex?page=" + page;
    }
    ...
    function editDepartment() {
        ...
        if (length !== 1) {
            ...
            window.location.href = "${path}/financeMaterial/toEdit?id=" + id + "&page=" + $('#page').val();
        }
    }
    ...
    function del() {
        ...
        if (length !== 1) {
            ...
            var id = $("tbody input:checked").val();
            //主页fun
            window.top.deleteFinanceMaterial(id, $('#page').val());
        }
    }
