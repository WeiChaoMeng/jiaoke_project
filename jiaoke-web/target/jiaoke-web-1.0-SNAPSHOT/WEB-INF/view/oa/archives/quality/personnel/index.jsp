            ...
            <td>
                <div>
                    <div class="conditional-query cursor_hand">
                        <input type="text" id="name" class="search-bar" placeholder="姓名" autocomplete="off">
...
<table class="simple_table cursor_hand">
    ...
    <th style="width: 32%;">证书编号</th>
    <th style="width: 32%;">颁发单位</th>
    </thead>
...
<script>
    ...
    function loadData(page) {
        $.ajax({
            type: "post",
            url: '/qualityPersonnel/loadingData',
            data: {'page': page},
            async: false,
    ...
    function searchButton(page, parameter) {
        ...
        $.ajax({
            type: "post",
            url: '/qualityPersonnel/searchFilter',
    ...
    function parseResult(resultLists) {
        ...
        if (resultList.length === 0) {
            ...
            for (let i = 0; i < resultList.length; i++) {
                ...
                result += '<td>' + resultList[i].position + '</td>';
                result += '<td>' + resultList[i].age + '</td>';
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].certificate + '">' + resultList[i].certificate + '</td>';
                result += '<td style="text-align:left;text-indent:10px" title="' + resultList[i].issuingUnit + '">' + resultList[i].issuingUnit + '</td>';
                result += '</tr>';
            }
        }
        $('#tbody').html(result);
    ...
    //新增
    function add() {
        window.location.href = '${path}/qualityPersonnel/toAdd?page=' + $('#page').val();
    }

    //详情
    function particulars(id) {
        window.location.href = "${path}/qualityPersonnel/details?id=" + id + "&page=" + $('#page').val();
    }
    ...
    function editDepartment() {
        ...
        if (length !== 1) {
            ...
            window.location.href = "${path}/qualityPersonnel/toEdit?id=" + id + "&page=" + $('#page').val();
        }
    }
