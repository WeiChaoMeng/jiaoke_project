...
<form id="transferTwo">
                ...
                <td style="width: 35%">
                    <div class="common_input_frame">
                        <input type="text" id="title" name="title" placeholder="请输入标题" title="点击此处填写标题"
                               value="${transferTwo.title}" autocomplete="off">
                    </div>
                </td>
    ...
    <span class="fill-in-date">填表日期：${transferTwo.createTimeStr}</span>
    ...
    <table class="formTable">
        ...
        <tr>
            ...
            <td class="table-td-content">
                <input type="text" class="formInput-readonly" name="name" value="${transferTwo.name}" readonly>
                <input type="hidden" id="id" name="id" value="${transferTwo.id}">
            </td>

            <td class="tdLabel">年龄</td>
            <td class="table-td-content">
                <input type="text" class="formInput" name="age" oninput="value=value.replace(/^(0+)|[^\d]/g,'')"
                       value="${transferTwo.age}" maxlength="2" autocomplete="off">
            </td>

            <td class="tdLabel">入职时间</td>
            <td class="table-td-content">
                    <input type="text" class="formInput entry-date" name="entryDateStr" value="${transferTwo.entryDateStr}" onfocus="this.blur()">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td colspan="2" class="table-td-content">
                    <input type="text" class="formInput" name="presentPost" value="${transferTwo.presentPost}" autocomplete="off">
            </td>

            <td class="tdLabel">拟转入岗位</td>
            <td colspan="2" class="table-td-content">
                <input type="text" class="formInput" name="newPost" value="${transferTwo.newPost}" autocomplete="off">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td colspan="5" class="table-td-evaluation">
                <textarea class="evaluation-content" style="height: 90px" name="cause">${transferTwo.cause}</textarea>
                <div class="approval-date">
                    <label class="approval-date-label">日期:</label>
                    <input class="approval-date-input" type="text" value="${transferTwo.createTimeStr}" disabled>
                </div>
                <div class="approval-signature">
                    <label class="approval-signature-label">本人签字:</label>
                    <input class="approval-signature-input" type="text" value="${transferTwo.promoterStr}" disabled>
                </div>
            </td>
        </tr>
...
<script>
    ...
    function send() {
        ...
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/transferTwo/editAdd',
                data: $('#transferTwo').serialize(),
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result === "success") {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '${path}/transferTwo/deleteAnnexes',
