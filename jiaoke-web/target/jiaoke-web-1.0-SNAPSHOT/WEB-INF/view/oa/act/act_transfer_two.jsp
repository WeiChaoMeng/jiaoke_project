...
<form id="oaActTransferTwo">
    <div class="form_area" id="titleArea" style="margin-bottom: 15px">
        <table>
            ...
            <tr>
                ...
                <td style="width: 30%">
                    <div class="common_input_frame">
                        <input type="text" id="title" name="title" placeholder="请输入标题" title="点击此处填写标题"
                               value="转岗审批表2(${nickname} <%=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())%>)"
                               autocomplete="off">
                    </div>
                </td>
                ...
                <td>
                    <div class="common_input_frame">
                        <input type="text" placeholder="发起者部门负责人(审批)、发起者部门主管领导(审批)、转入部门负责人(审批)、转入部门主管领导(审批)、组织人事部(审批)、总经理(审批)、发起人(协同)"
                               readonly="readonly">
                    </div>
                </td>
            </tr>
    ...
    <table class="formTable">
        ...
        <tr>
            ...
            <td class="table-td-content">
                <input type="text" class="formInput" name="age" oninput = "value=value.replace(/^(0+)|[^\d]/g,'')" maxlength="2" autocomplete="off">
            </td>
        ...
        <tr>
            ...
            <td colspan="2" class="table-td-content">
                <input type="text" class="formInput" name="newPost" autocomplete="off">
            </td>
        </tr>
        ...
        <tr>
            ...
            <td colspan="5" class="table-td-evaluation">
                <textarea class="evaluation-content" style="height: 90px" name="cause"></textarea>
...
<script>
    ...
    function send() {
        ...
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/transferTwo/add',
                data: $('#oaActTransferTwo').serialize(),
    ...
    function savePending() {
        ...
        if ($.trim($("#title").val()) === '') {
            ...
            $.ajax({
                type: "POST",
                url: '${path}/transferTwo/savePending',
                data: $('#oaActTransferTwo').serialize(),
                error: function (request) {
                    layer.msg("出错！");
                },
